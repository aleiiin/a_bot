...
for admin in ADMINS:
    ...
    if not cur.fetchone():
        cur.execute('INSERT INTO Users VALUES (?, ?, ?, ?, ?, ?, ?)', (admin, 0, get_date(), 0, 0, 1, 0))
...
async def start(msg: Message):
    ...
    if result is None:
        date = get_date()
        if user in ADMINS:
            cur.execute("INSERT INTO Users VALUES(?, ?, ?, ?, ?, ?, ?);", (user, 0, date, 0, 0, 1, 0))
            cur.execute("INSERT INTO Admins VALUES(?, ?, ?, ?, ?);", (user, date, 1, 1, 0))
        else:
            cur.execute("INSERT INTO Users VALUES(?, ?, ?, ?, ?, ?, ?);", (user, 0, date, 0, 0, 0, 0))
    if user in ADMINS:
